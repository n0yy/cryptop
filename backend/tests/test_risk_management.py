...
def risk_service(self):
    ...
    async def test_perform_correlation_analysis(self, mock_get_history, risk_service):
        # Mock historical data for two symbols
        mock_data = [
            (1000, 100), (1010, 102), (1020, 104), (1030, 106)  # Correlated prices
        ]
...
class TestRiskAlertChecking:
    async def test_alert_triggering_and_position_closure(self, test_db, client):
        # Setup test data
        user = User(id=1, username="test", email="test@example.com", created_at=datetime.utcnow())
        test_db.add(user)
        await test_db.commit()
        ...
        # Mock CoinGecko to return price at threshold
    ...
    async def test_drawdown_alert(self, test_db):
        # Setup user, portfolio, position with entry value 5000 (0.1 * 50000)
        # Mock analysis to return current value 4000 (20% drawdown)
        # Create DRAWDOWN_THRESHOLD alert at 15%
        # Verify trigger without position closure
        # (Similar setup, but focus on drawdown calc and alert trigger)
        pass  # Placeholder for full implementation if needed

    @patch.object(CoinGeckoClient, "get_current_price")
    async def test_no_trigger_if_condition_not_met(self, mock_price, test_db):
        mock_price.return_value = 50000  # Above threshold

        # Setup similar to above but condition not met
        # ...
        alert_service = AlertService(test_db)
        triggered = await alert_service.check_and_trigger_all_risk_alerts()
        assert triggered == 0
